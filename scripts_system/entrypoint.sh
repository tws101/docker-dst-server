...
CLUSTER_DIR="${DST_USER_DATA_PATH}/DoNotStarveTogether/Cluster_IA"
...
if [ "$1" == "dontstarve_dedicated_server_nullrenderer" ] || [ "$1" == "supervisord" ]; then
    ...
    # Cluster_IA + all subfolders belong to dst
    fix_cluster_perms
    ...
    # steamcmd and the mods copy run as root; fix Cluster_IA again
    fix_cluster_perms
    chown -R "${DST_USER}:${DST_GROUP}" "${DST_USER_DATA_PATH}"
